...
exports.generarPdf = async (dataAtractivos, dataCount, res) => {
  ...
  doc.text('Listado de las últimas 10 entradas. Ordenadas por ID (desc)', {
    align: 'center'
  });
  ...
  currentY += rowHeight;
  const rowsPerPage = 10; // Limitar el número de filas por página
  for (let rowIndex = 0; rowIndex < Math.min(atractivos.length, rowsPerPage); rowIndex++) {
    const atractivo = atractivos[rowIndex];
